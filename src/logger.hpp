...
namespace wissbi { namespace logger {

static std::mutex mutex;

namespace priv {
template <typename T>
void dispatch_r(std::list<std::string>::iterator iter, std::list<std::string>::iterator end, std::ostringstream& oss, const T& last) {
    oss << *iter << last;
    oss << *(++iter);
    if(++iter != end) {
...
void dispatch_r(std::list<std::string>::iterator iter, std::list<std::string>::iterator end, std::ostringstream& oss, const U& head, const T&... tail) {
    oss << *iter << head;
...
void log(const std::string& format, const U& head, const T&... tail)
{
    ...
    do {
        ...
        segments.push_back(format.substr(seg_begin_pos, seg_end_pos - seg_begin_pos));
