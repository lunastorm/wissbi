...
typedef struct FilterMetric {
  FilterMetric() : name(""), last_processed(0), total_processed(0) {}

  explicit FilterMetric(const std::string &in_name)
      : name(in_name), last_processed(0), total_processed(0) {}

  FilterMetric(const FilterMetric &rhs)
      : name(rhs.name), last_processed(rhs.last_processed.load()),
