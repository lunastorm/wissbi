...
public:
  Connector(const std::string dest, T &input_filter, MetricList &metric_list,
            std::atomic_uint &pending_counter) {
    std::thread(ConnectorLoop, dest, std::ref(input_filter),
                std::ref(metric_list), std::ref(pending_counter)).detach();
  }

  ~Connector() {}

private:
  static void ConnectorLoop(const std::string dest, T &filter,
                            MetricList &metric_list,
                            std::atomic_uint &pending_counter) {
    std::unordered_set<std::string> producer_set;

    SubDir sub_dir(getenv("WISSBI_META_DIR") != NULL ? getenv("WISSBI_META_DIR")
                                                     : "/var/lib/wissbi",
                   dest);
    while (true) {
      for (auto sub_entry_tuple : sub_dir.GetSubList()) {
        ...
        auto iter = std::find_if(metric_list.begin(), metric_list.end(),
                                 [real_dest](const FilterMetric & m) {
        ...
        std::thread(
            [conn_str,
                dest, real_dest, &producer_set, &metric, &pending_counter] {
          sockaddr sock_addr;
          util::ConnectStringToSockaddr(
              conn_str, reinterpret_cast<sockaddr_in *>(&sock_addr));
          ...
          producerFilter.set_post_filter_func(
              [dest, real_dest, &metric, &pending_counter, &producerFilter](
                  bool filter_result, MsgBuf & msg_buf) {
            if (filter_result == true) {
              ...
              static_cast<InputWrapper<io_policy::SysvMq> &>(producerFilter)
                  .Put(msg_buf);
              return false;
            }
          });
