...
void sleep_while(std::function<bool()> cond, int second) {
  ...
  while (cond()) {
    if (second <= chrono::duration_cast<chrono::seconds>(
                      chrono::system_clock::now() - wait_start_tp).count()) {
      break;
    }
...
typedef MsgFilter<io_policy::Line, io_policy::Tee<io_policy::SysvMq> >
    StdLineInputFilter;
typedef MsgFilter<io_policy::Length, io_policy::Tee<io_policy::SysvMq> >
    StdLengthInputFilter;
...
void run(const std::string &dest, int wait_timeout_sec) {
  ...
  Connector<InputFilter> connector(dest, input_filter, metric_list,
                                   pending_counter);

  MetricInputFilter metric_input_filter;
  list<FilterMetric> dummy_metric_list;
  atomic_uint metric_pending_counter(0);
  Connector<MetricInputFilter> metric_connector(
      "wissbi.metric", metric_input_filter, dummy_metric_list,
      metric_pending_counter);
  MetricReporter metric_reporter(metric_list, metric_input_filter, "enqueue");
  metric_input_filter.set_post_filter_func(
      [&metric_input_filter, &metric_pending_counter](bool filter_result,
                                                      MsgBuf & msgbuf) {
  ...
  MetricReporter drop_metric_reporter(drop_metric_list, metric_input_filter,
                                      "drop");
  ...
  input_filter.set_pre_filter_func([&input_filter, wait_timeout_sec] {
    sleep_while([&input_filter] {
      return input_filter.GetBranchCount() == 0;
    },
                wait_timeout_sec);
    return true;
  });
  input_filter.set_post_filter_func(
      [dest, &input_filter, &pending_counter, &drop_metric_list](
          bool filter_result, MsgBuf & msgbuf) {
    if (filter_result == true) {
      pending_counter += input_filter.GetLastTeeCount();
      drop_metric_list.begin()->last_processed +=
          input_filter.GetLastFailedCount();
    }
    return filter_result;
  });
  ...
  sleep_while([dest, &pending_counter] {
    return pending_counter > 0;
  },
              wait_timeout_sec);

  metric_reporter.Report();
  sleep_while([&metric_pending_counter] {
    return metric_pending_counter > 0;
  },
              wait_timeout_sec);
}
